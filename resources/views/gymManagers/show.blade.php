...
@section('title', "View Gym Manager Information")

@section('content')
<div class="container p-5">
    <div class="card card-info">
        <div class="card-header">
            <h3 class="card-title">Gym Manager Info : <b>{{ $manager->name }}</b></h3>

            <div class="card-tools row">
                <div class="col-md-4">
                    <a class="btn btn-tool btn-info" href="{{ route('gymManagers.edit', $manager->id) }}"><i class="fas fa-pencil-alt"></i></a>
                </div>
                <form class="col-md-4" action="{{ route('gymManagers.destroy', $manager->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
        ...
        <div class="card-body">
            ...
            <p class="card-text text-secondary">City : <span class="text-light font-weight-bold">{{$manager->gym->city->name}}</span> </p>
            <p class="card-text text-secondary">Gym : <span class="text-light font-weight-bold">{{$manager->gym->name}}</span> </p>
        </div>
    </div>
</div>
@stop
